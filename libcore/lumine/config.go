...
// GetDefaultPolicy returns a pointer to the default policy
func GetDefaultPolicy() *Policy {
	return &defaultPolicy
}

// GetIPPolicy returns the policy for a given IP address (exported version)
func GetIPPolicy(ip string) *Policy {
	return getIPPolicy(ip)
}

// applyConfig applies a configuration without reading from file
func applyConfig(conf *Config) error {
	...
	if conf.FakeTTLRules != "" {
		err := loadFakeTTLRules(conf.FakeTTLRules)
		if err != nil {
			return fmt.Errorf("parse fake TTL rules: %s", err)
		}
	}

	if conf.TransmitFileLimit > 0 {
		sem = make(chan struct{}, conf.TransmitFileLimit)
	}
